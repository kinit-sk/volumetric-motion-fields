...
class RainNet3D(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.mode == "regression":
            self.last_layer = nn.Conv3d(conv_shape[-1][-1][-1], self.im_shape[1], kernel_size=1, padding = 'valid', padding_mode='reflect')
    ...
    def make_conv_block(self, in_ch, out_ch, kernel_size):
        ...
        return nn.Sequential(
            ...
            spectral_norm(nn.Conv3d(in_ch, out_ch, kernel_size, padding='same', padding_mode='reflect')),
            nn.BatchNorm3d(out_ch),
            nn.ReLU(),
            spectral_norm(nn.Conv3d(out_ch, out_ch, kernel_size, padding='same', padding_mode='reflect')),
            nn.ReLU()
            )
    ...
    def forward(self, x):
        ...
        x = self.pool_final(torch.cat((self.upsample((self.conv["8"](x))), x1s), dim=1)) # up9
        x = self.pool_final((self.conv["9"](x))) #conv9
        x = self.pool_final((self.last_layer(x))) #outputs
