...
class MF3DUNetAltitudeWise(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        x, y, _ = batch
        x = x[:,:,0].permute(0,2,1,3,4).float()
        y = y[:,:,0].permute(0,2,1,3,4).float()

        x = x.reshape(x.shape[0]*x.shape[1],*x.shape[2:]).unsqueeze(1)
        y = y.reshape(y.shape[0]*y.shape[1],*y.shape[2:]).unsqueeze(1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            y_hat = y_hat.reshape(len(_), y_hat.shape[0] // len(_), *y_hat.shape[1:])
            fig, axes = plt.subplots(4, 4, figsize=(16,16), layout='constrained')
            for i in range(len(y_hat)):
                for j in range(4):
                    plot_precip_field(y_hat[:4,0,0:16:4][i][j].cpu().numpy(), colorbar=False, units='mm/h', ax=axes[i,j])
                    quiver(mf.squeeze(2)[y_hat.shape[1]*i].cpu().numpy(), step=16, ax=axes[i,j])
    ...
    def test_step(self, batch, batch_idx):
        ...
        y_hat = y_hat.reshape(len(_), y_hat.shape[0] // len(_), *y_hat.shape[1:])
        y = y.reshape(len(_), y.shape[0] // len(_), *y.shape[1:])

        pearson = PearsonCorrCoef().to(mf.device)
        motion_field_1km = mf[1::y_hat.shape[1]]
        motion_field_3km = mf[5::y_hat.shape[1]]

        pearson_corr = pearson(motion_field_1km.flatten(), motion_field_3km.flatten())
        mse_mf = torch.nn.functional.mse_loss(motion_field_1km, motion_field_3km)
        self.log("test/PearsonCorr_1km_3km", pearson_corr.detach())
        self.log("test/MSE_MotionField_1km_3km", mse_mf.detach())
        ...
        if batch_idx % 100 == 0:
            ...
            for i in range(len(y_hat)):
                for j in range(4):
                    plot_precip_field(y_hat[:4,0,0:16:4][i][j].cpu().numpy(), colorbar=False, units='mm/h', ax=axes[i,j])
                    quiver(mf[y_hat.shape[1]*i].cpu().numpy(), step=16, ax=axes[i,j])
                    axes[i,j].set_xticks(np.arange(0, y_hat.shape[-2], 64))
                    axes[i,j].set_yticks(np.arange(0, y_hat.shape[-1], 64))
                    axes[i,j].grid(True)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        y_hat = self.trainer.datamodule.test_dataset.invScaler(y_hat)
        y_hat[y_hat < 0] = 0
        y_hat[torch.isnan(y.squeeze())] = np.nan

        y_hat = y_hat.reshape(len(_), y_hat.shape[0] // len(_), *y_hat.shape[1:]).permute(0,2,1,3,4)
