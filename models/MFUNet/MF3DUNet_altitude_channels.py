...
class MF3DUNetAltitudeChannels(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        y_hat = torch.zeros((x.shape[0], x.shape[1], leadtimes, *x.shape[3:])).to(x.device)
        mf = mf.reshape(mf.shape[0], x.shape[1], 2, *mf.shape[3:])
        for i in range(x.shape[1]):
            y_hat[:,i] = self._extrapolate(leadtimes, x[:,i:i+1,-1], mf[:,i], self.trainer.state.stage)
        return y_hat, mf
    ...
    def training_step(self, batch, batch_idx):
        ...
        for p in range(8):
            for a in range(x.shape[1]):
                x_pool = F.avg_pool2d(x[:,a], kernel_size=2**p, stride=2**p)
                y_pool = F.avg_pool2d(y[:,a], kernel_size=2**p, stride=2**p)
                mf_pool = F.avg_pool2d(mf[:,a], kernel_size=2**p, stride=2**p)
                mf_pool = mf_pool / (2**p)

                sequence = torch.cat([x_pool, y_pool], dim=1)
                for i in range(sequence.shape[1]-1):
                    extrapolated = self._extrapolate(1, sequence[:,i:i+1], mf_pool, self.trainer.state.stage)
        ...
        if batch_idx == 0:
            ...
            for i in range(len(y_hat)):
                for j in range(4):
                    ...
                    quiver(mf[i,0].cpu().numpy(), step=16, ax=axes[i,j])
